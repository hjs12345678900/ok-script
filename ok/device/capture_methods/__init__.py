"""Capture backends exposed without importing foreign-platform native modules."""
...
from ok.device.capture_methods.types import (
    ColorChannel,
    ImageShape,
    decimal,
    is_digit,
...
if sys.platform == "win32":
    from ok.device.capture_methods.bitblt import (
        BitBltCaptureMethod,
    ...
    from ok.device.capture_methods.browser import (
        BrowserCaptureMethod,
        BrowserWGC,
        BrowserWindowAdapter,
    )
    from ok.device.capture_methods.desktop_duplication import (
        DesktopDuplicationCaptureMethod,
    )
    from ok.device.capture_methods.hwnd_window import (
        HwndWindow,
        check_pos,
        get_monitors_bounds,
        get_mute_state,
    ...
    from ok.device.capture_methods.update import (
        get_capture,
    ...
    from ok.device.capture_methods.macos import MacScreenCaptureMethod
